...
public class YakubSpawnHandler {
    ...
    private static void handleWorldSpawns(ServerWorld world) {
        if (world == null || !canSpawn) {
            return;
        }

        // Check if we're in Agartha dimension
        boolean isAgartha = world.getRegistryKey().equals(ModDimensions.AGARTHA_WORLD_KEY);
        ...
        for (ServerPlayerEntity player : world.getPlayers()) {
            ...
            BlockPos spawnPos = findValidSpawnPos(world, player.getBlockPos());
            if (spawnPos != null) {
                ...
                yakub.setYaw(world.getRandom().nextFloat() * 360);
                yakub.setPitch(0);

                if (world.spawnEntity(yakub)) {
                    canSpawn = false;
    ...
    private static BlockPos findValidSpawnPos(ServerWorld world, BlockPos center) {
